...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, text):
        super().__init__()
        self.tokenizer = AutoTokenizer.from_pretrained("internlm/internlm-chat-7b")
        self.text = text
...
class OptModel(torch.nn.Module):
    def __init__(self):
        ...
        print('Model loaded from pretrained internlm-chat-7b:')
        print(self.model)
        print("---------------------------------------")
...
# Save the model
torch.save(model.state_dict(), 'internlm_chat_7b_model.pth')
print('Model saved to internlm_chat_7b_model.pth')
